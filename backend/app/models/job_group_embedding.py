...
class JobGroupEmbedding(Base):
    __tablename__ = "job_group_embeddings"
    __table_args__ = (
        UniqueConstraint("profile_id", "group_key", name="uq_job_group_embedding_profile_group"),
        UniqueConstraint("vector_id", name="uq_job_group_embedding_vector"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    profile_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("job_requirement_profiles.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
