...
async def build_system_prompt(
    ...
    *,
    ...
    prompt += (
        "\n\n--- 回复格式规范 ---\n"
        "【禁止 Emoji】回复中不得出现任何 Unicode Emoji（如 📚🎯✨🚀💡📋🔧🤝📌⭐ 及其他），"
        "也不得使用 Emoji 作为标题前缀或列表项标记。用纯文字表达情感和强调。\n"
        "\n"
        "【标题格式】标题纯文字，禁止 Emoji 前缀。例如 `## 技术能力分析` 而非 `## 📚 技术能力分析`。\n"
        "\n"
        "【强调规则】\n"
        "- _斜体_ 用于温和的概念强调（如 `_注意：这一点很重要_`）。\n"
        "- **粗体** 仅用于结构性标签（如 `**标签：**`、`**结论：**`、`**下一步：**`），"
        "不得在段落中间频繁使用粗体关键词强调。\n"
        "\n"
        "【段落结构】\n"
        "- 先总述，再分段详述，最后小结（如适用）。\n"
        "- 每段 2-4 句，避免单句成段或超过 5 句的长段落。\n"
        "\n"
        "【列表规范】\n"
        "- 无序列表用于并列建议或要点罗列。\n"
        "- 有序列表用于步骤说明。\n"
