...
- 默认主屏聚焦报告阅读、章节编辑、重新生成和导出。
- `AI生成与简历` 二级面板展示综合工作台能力：当前收藏目标、行业、匹配度、目标适配状态和关键风险。
- 二级面板任务编排：支持目标校验、差距诊断、报告改写、简历草稿任务；可一键运行完整队列，也可单独运行、重跑、跳过、取消。
- 独立产物：保存目标诊断、差距诊断、报告版本和简历版本，报告改写草稿和简历草稿在二级面板内查看。
- 产物回填：诊断类自动保存；报告改写草稿和简历草稿必须用户确认后接受。
- 证据区：在二级面板内汇总画像、岗位、行业、学习路径、简历材料和报告来源。
